<template>
  <div class="chat-main" >
    ...
    <div class="chat-content"  @click="scrollEvent" style="height:220px">
...
// 滚动条到底事件
    scrollEvent(e){
      ...
      console.log('+++++++++++++',e.srcElement)
        ...
        if (e.srcElement.scrollTop  == 0 ) {
          alert(1)
          if(that.pageData.current_page>2){
            // that.pageData.current_page--
            // this.sendWs.send=function(){
            //   JSON.stringify({
            //     action: 'historicalData',
            //     params: {
    ...
    update: function() {
      ...
      this.sendWs.onmessage = function(_msg) {
        // 接收语音记录
        if (JSON.parse(_msg.data).identification == 'historicalData') {
          console.log(JSON.parse(_msg.data).data)
          // that.recordContent = JSON.parse(_msg.data).data.data.reverse() // 记录数据更新
          that.recordContent = JSON.parse(_msg.data).data.data // 记录数据更新
          // that.pageData = that.recordContent.page.current_page
        }
      }
      this.$nextTick(() => {
